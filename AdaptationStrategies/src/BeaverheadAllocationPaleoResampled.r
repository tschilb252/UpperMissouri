...
datMeasAggDist = datMeasAgg %>%
  filter(Measure == 'East Bench Allocation', Strategy == 'Baseline')

datMeasAggComp = datMeasAgg %>%
  filter(Measure == 'CCWSC Allocation', Strategy == 'Baseline')
...
test2 = datMeasAggDist %>% filter(Trace == 139)
...
ggsave(plot = Plot3, paste0(dirOup, 'BeaverheadAllocationLongestDrought.png'), height = 8, width = 15)
